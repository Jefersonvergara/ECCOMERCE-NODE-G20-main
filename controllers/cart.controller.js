...
exports.buyProductOnCart = catchAsync(async (req, res, next) => {
  ...
  //vamos a actualizar el stock 
  const purchasedProductPromises = cart.productInCarts.map(
    async productInCart => {
      //buscar el producto para actualizar 
      const product = await Product.findOne({
        where: {
          id: productInCart.productId,
        },
      });
      // calcular la cantidad de productos que me quedan en la tiendan
      
      const newStock = product.quantity - productInCart.quantity;
      //actualizabamos la informacion y la retornabamos
